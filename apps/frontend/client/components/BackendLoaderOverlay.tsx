...
/**
 * Full-screen overlay that blocks UI until the backend is ready.
 * Features a beautiful 3D server animation with sequential loading messages.
 */
export default function BackendLoaderOverlay() {
  const [isReady, setIsReady] = useState(false);
  const [currentMessage, setCurrentMessage] = useState(0);
  const [timeoutReached, setTimeoutReached] = useState(false);

  const loadingMessages = [
    { text: "Starting server", duration: 10000 },
    { text: "Loading Database", duration: 10000 },
    { text: "Preparing Migrations", duration: 10000 },
    { text: "Endpoints Starting", duration: 10000 },
  ];
  ...
  useEffect(() => {
    ...
    async function waitForBackend() {
      ...
      // If timed out, allow UI but inform user
      if (mounted) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center">
      <div className="flex flex-col items-center gap-6 p-8 rounded-lg bg-background shadow-lg border max-w-md w-full mx-4">
        {/* 3D Server Animation */}
        <div className="server-container">
          <div className="server-main">
            <svg width={74} height={90} viewBox="0 0 74 90" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path className="strobe-color-1" d="M58 72.5L60.5 71V74L58 75.5V72.5Z" fill="#FF715E" />
              <path className="strobe-color-2" d="M63 69.5L65.5 68V71L63 72.5V69.5Z" fill="#17e300b4" />
              ...
              <path className="strobe-color-1" d="M63 51.5L65.5 50V53L63 54.5V51.5Z" fill="#FF715E" />
              ...
              <path className="strobe-color-3" d="M58 36.5L60.5 35V38L58 39.5V36.5Z" fill="#FF715E" />
              ...
              <path className="strobe-led-1" d="M55.1521 27.1464L53.2538 26.054L55.3602 24.9661L57.2585 26.0586L55.1521 27.1464Z" fill="#3A6DAB" />
            </svg>
          </div>
          <div className="server-ghost">
        ...
        <div className="text-center space-y-2">
          <h3 className="text-lg font-semibold text-foreground message-text">
            {loadingMessages[currentMessage].text}
            <span className="loading-dots">.</span>
          </h3>
          <p className="text-sm text-muted-foreground">
            {timeoutReached 
              ? "Backend is taking longer than expected. You can refresh the page manually."
              : "Please wait while we initialize the backend services..."
            }
          </p>
        </div>
      ...
      <style>{`
        ...
        .strobe-color-1 {
          animation: strobe-color-2 0.8s infinite;
        }

        .strobe-color-3 {
          animation: strobe-color-1 0.8s infinite;
          animation-delay: 3s;
        }
        ...
        .message-text {
          animation: messageSlide 10s infinite ease-in-out;
        }
